// patcher.rs
//
// Implements a patcher for the Azure DevOps OpenAPI specs.
use json::JsonValue;
...
// Spec description replacements for specific files.
...
impl Patcher {
    ...
    pub fn new(spec_path: &Path) -> Patcher {
        Patcher {
    ...
    fn patch_array_array_schema(&mut self, key: &[&str], value: &JsonValue) -> Option<JsonValue> {
        match key {
            ["paths" | "x-ms-paths", _path, _op, "responses", _rsp_code, "schema"] => {
                // memberEntitlementManagement.json has this, which declares an array but omits the item's type:
                // "responses": {
                //     "200": {
                //       "description": "successful operation",
                //       "schema": {
    ...
    /// Patch JObject
    ///
    /// These are actually arbitrary JSON objects.
    fn patch_jobjects(&mut self, _key: &[&str], value: &JsonValue) -> Option<JsonValue> {
        if let JsonValue::Object(obj) = value {
            if let Some("#/definitions/JObject") = obj.get("$ref").and_then(|s| s.as_str()) {
                let mut value = value.clone();
                value.remove("$ref");
                if value
    ...
    // Each patch function can replace the existing value by returning `Some<JsonValue>`,
    ...
    // Add all the accumulated new definitions to the definitions section of the schema.
    ...
    // Main entry point.
    //
    // Walk the JSON object tree, invoking the patcher for each key/value pair.
    pub fn run(&mut self, json: &mut JsonValue) {
        self.walker(&[], json);
