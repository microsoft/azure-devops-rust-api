...
pub fn run(config: Config) -> Result<()> {
    ...
    if config.should_run(&Runs::Operations) {
        let operations =
            codegen_operations::create_operations(cg).map_err(Error::CreateOperations)?;
        let operations_path =
            path::join(&config.output_folder, "operations.rs").map_err(Error::Path)?;
        ...
        if let Some(api_version) = cg.spec.api_version() {
            ...
            let operations_path =
                path::join(&config.output_folder, "mod.rs").map_err(Error::Path)?;
            write_file(&operations_path, &operations, config.print_writing_file)?;
        } else {
...
fn get_readme(
    spec_folder_full: &dyn AsRef<Path>,
    readme_kind: &dyn AsRef<Path>,
) -> Option<PathBuf> {
