use crate::{
    codegen::{
        add_option, create_generated_by_header, enum_values_as_strings, is_basic_type,
        is_local_enum, is_local_struct, is_vec, type_name_gen, Error,
    },
...
fn create_basic_type_alias(
    ...
    let id = property_name
        .to_camel_case_ident()
        .map_err(Error::StructName)?;
    let value = type_name_gen(
        &get_type_name_for_schema(&property.schema.common)?,
        false,
        false,
    )?;
    Ok((id, value))
...
fn create_struct(
    ...
    for (property_name, property) in &properties {
        ...
        let (mut field_tp_name, field_tp) = create_struct_field_type(
            cg,
            doc_file,
            &ns,
            property_name,
        ...
        let is_required =
            required.contains(property_name.as_str()) && !cg.should_force_optional(prop_nm);
    ...
    match &property.ref_key {
        Some(ref_key) => {
            let tp = ref_key
                .name
                .to_camel_case_ident()
                .map_err(Error::PropertyName)?;
            Ok((tp, Vec::new()))
        }
        None => {
            if is_local_enum(property) {
                let (tp_name, tp) =
                    create_enum(namespace, property_name, property, lowercase_workaround)?;
                Ok((tp_name, vec![tp]))
