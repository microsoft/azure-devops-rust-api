...
impl SchemaGen {
    ...
    fn xml_name(&self) -> Option<&str> {
        self.schema
            ...
            .xml
            .as_ref()
            .and_then(|xml| xml.name.as_deref())
    }
...
fn resolve_schema_properties(
    ...
    schema
        ...
        .all_of
        .iter()
        .for_each(|ref_or_schema| match ref_or_schema {
...
fn resolve_schema_property(
    ...
    let schema = if let Some(ref_key) = &property.ref_key {
        if let Some(schema) = resolved.get(ref_key) {
            ...
            let schema = all_schemas.get(ref_key).ok_or_else(|| {
                Error::with_message(ErrorKind::CodeGen, || {
                    format!("ref key not found {ref_key:?}")
                })
            })?;
            // prevent overflow for recursive call
            resolved.insert(ref_key.clone(), schema.clone()); // unresolved properties
            let schema =
                resolve_schema_properties(resolved, all_schemas, spec, &ref_key.file_path, schema)?;
...
fn resolve_all_schema_properties(
    schemas: &IndexMap<RefKey, SchemaGen>,
    spec: &Spec,
) -> Result<IndexMap<RefKey, SchemaGen>> {
    ...
    for (ref_key, schema) in schemas {
        ...
        let schema =
            resolve_schema_properties(&mut resolved, schemas, spec, &ref_key.file_path, schema)?;
        resolved.insert(ref_key.clone(), schema);
    }
    Ok(resolved)
}

fn resolve_all_all_of(
    schemas: &IndexMap<RefKey, SchemaGen>,
    spec: &Spec,
) -> Result<IndexMap<RefKey, SchemaGen>> {
    let mut resolved: IndexMap<RefKey, SchemaGen> = IndexMap::new();
        ...
        if let Some(pageable) = operation.pageable.as_ref() {
            for response in operation.responses.values() {
                if let Some(schema) = &response.schema {
                    let pageable_name =
                        TypeNameCode::new(&get_type_name_for_schema_ref(schema)?)?.to_string();
    ...
    for (ref_key, schema) in all_schemas {
        ...
        if let Some(_first_doc_file) = schema_names.insert(schema_name, doc_file) {
            ...
            if let Some(tag) = schema.discriminator() {
                ...
                let mut schema = schema.clone();
                let tag_property = schema
                    .properties
                    .iter()
                    .find(|property| property.name() == tag);
                let tag_property_description = tag_property
                    .and_then(|property| property.schema().schema.common.description.clone());
...
impl UnionCode {
        ...
        for (child_ref_key, child_schema) in all_schemas {
            if let Some(tag) = child_schema.discriminator_value() {
                if Self::breadth_first_search_all_of(ref_key, child_schema) {
                    ...
                    let mut type_name =
                        TypeNameCode::from(child_ref_key.name.to_camel_case_ident()?);
                    cg.set_if_union_type(&mut type_name);
    ...
    fn populate_heap(
        heap: &mut BinaryHeap<Reverse<Depth<SchemaGen>>>,
        schema: &SchemaGen,
        depth: usize,
    ) {
        if depth != 0 {
...
fn create_basic_type_alias(
    cg: &CodeGen,
    property_name: &str,
    property: &SchemaGen,
) -> Result<TypeAliasCode> {
...
fn create_struct(
    ...
    for property in schema.properties() {
        ...
        if is_required {
            if type_name.is_date_time() {
                ...
                serde.add_with("crate::date_time::rfc3339");
        ...
        doc_comments.push(DocCommentCode::from(
            &property.schema.schema.common.description,
        ));
    ...
    let continuable =
        ContinuableCode::from_pageable(struct_name_code.clone(), pageable, field_names)?;
...
impl ContinuableCode {
    ...
    pub fn from_pageable(
        struct_name: Ident,
        pageable: Option<&MsPageable>,
        field_names: HashMap<String, bool>,
    ) -> Result<Option<Self>> {
        if let Some(pageable) = pageable {
            ...
            let is_required = field_name
                .as_ref()
                .and_then(|field_name| field_names.get(&format!("{field_name}")));
            Ok(Some(Self::new(
                struct_name,
                field_name,
                is_required.cloned(),
...
impl SerdeCode {
    ...
    pub fn add_skip_serializing_if(&mut self, skip_serializing_if: &str) {
        self.attributes
            .push(quote! { skip_serializing_if = #skip_serializing_if });
    }
    ...
    pub fn add_deserialize_with(&mut self, deserialize_with: &str) {
        self.attributes
            .push(quote! { deserialize_with = #deserialize_with });
    }
    pub fn add_serialize_with(&mut self, serialize_with: &str) {
        self.attributes
            .push(quote! { serialize_with = #serialize_with });
    }
...
mod union_code_tests {
    ...
    fn setup_scenario() -> HashMap<&'static str, (RefKey, SchemaGen)> {
        ...
        schema_2a.1.schema.x_ms_discriminator_value =
            Some("schema_2a_discriminator_value".to_string());
        ...
        schema_2b.1.schema.x_ms_discriminator_value =
            Some("schema_2b_discriminator_value".to_string());

        schema_2c.1.all_of = vec![schema_1b.1.clone()];

        schema_3a.1.all_of = vec![schema_2a.1.clone()];
        schema_3a.1.schema.x_ms_discriminator_value =
            Some("schema_3a_discriminator_value".to_string());
        schema_3b.1.all_of = vec![schema_2b.1.clone()];
        schema_3b.1.schema.x_ms_discriminator_value =
            Some("schema_3b_discriminator_value".to_string());
    ...
    fn test_breadth_first_search_all_of() {
        ...
        assert_eq!(
            UnionCode::breadth_first_search_all_of(
